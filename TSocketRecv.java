...
public class TSocketRecv extends TSocket_base {
    ...
    public int receiveData(byte[] buf, int offset, int length) {
        ...
        try {
            ...
            int numBytes = 0;
            while (length > numBytes && !rcvQueue.empty()) {
                numBytes += this.consumeSegment(buf, offset + numBytes, length - numBytes);
            }
            return numBytes;
        } finally {
            lock.unlock();
    ...
    public void processReceivedSegment(TCPSegment rseg) {
        ...
        try {
            printRcvSeg(rseg);
            //si hay espacio lo ponemos(put y desp signal) en la cola sino lo tiro
            if (!rcvQueue.full()) {
                rcvQueue.put(rseg);
                appCV.signalAll();
            }
